...
import { useTestVisibilityState } from '@/state/visibility';
import { useExitListener } from '@/hooks/useExitListener';
import { useNuiEvent } from '@/hooks/useNuiEvent';

const Test: React.FC = () => {
  const [visible, setVisible] = useTestVisibilityState();
      ...
      {shouldRender && (
        ...
        >
          <div className="flex h-full w-full flex-col items-center justify-center gap-2 rounded-lg border bg-background p-2">
            ...
            <p className="text-sm text-muted-foreground">This is a placeholder for the TEST UI.</p>
          </div>
        </div>
      )}
    </>
